			...
			data.forEach(obj => {
				...
				if (chartType !== CHART_TYPE.RADAR) {
					...
					obj.values.forEach(function (value, index) {
						var dataLabelF = (typeof obj.labelsdata !== 'undefined' && typeof obj.labelsdata[index] !== 'undefined') ? ' '+obj.labelsdata[index] : '';
						strXml += "  <c:dLbl>";
						strXml += '  <c:idx  val=\"' + index + '\"/>';
						strXml += "    <c:numFmt formatCode=\"".concat(encodeXmlEntities(opts.dataLabelFormatCode.replace('{X}',dataLabelF)) || 'General', "\" sourceLinked=\"0\"/>");
